...
1. Path-based rules
   - `SKILL.md` is `entry`
   - `*.contract.json` is `contract`
   - `*.catalog.json` is `catalog`
...
- A compiler-facing contract is `contract`, not `schema`.
- A machine-readable validation catalog is `catalog`, not `schema`.
